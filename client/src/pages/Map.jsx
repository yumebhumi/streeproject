...
const Map = () => {
  ...
  useEffect(() => {
    const fetchIncidents = async () => {
      try {
        ...
        const response = await axios.get(`${backendUrl}/api/incidents/getAllIncidents`, config);
  ...
  const filteredIncidents = useMemo(() => {
    return incidents.filter((incident) => category ? incident.category === category : true);
  }, [category, incidents]);

  const summary = useMemo(() => {
    const counts = filteredIncidents.reduce((accumulator, incident) => {
    ...
    const latestIncident = [...filteredIncidents].sort((left, right) => new Date(right.date) - new Date(left.date))[0];
